...
app.post('/decrypt-and-forward', async (req, res) => {
	...
	try {
		const { ciphertext, nonce, userId, templateId, recipient } = req.body;

		// Validate required fields
		if (!ciphertext || !nonce || !userId || !templateId || !recipient) {
			return res.status(400).json({
				error: 'Missing required fields',
				required: ['ciphertext', 'nonce', 'userId', 'templateId', 'recipient']
			});
		}
		...
		const plaintext = await decrypt(ciphertext, nonce, userId);

		console.log('Message decrypted successfully (length:', plaintext.length, 'bytes)');

		// 2. Forward plaintext to CWC API
		...
		// 3. Generate attestation token for response
