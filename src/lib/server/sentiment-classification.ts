...
export async function classifyMessageSentiment(text: string): Promise<SentimentResult> {
  ...
  const proKeywords = [
    'support', 'approve', 'endorse', 'favor', 'agree', 'yes', 'forward', 'progress',
    'reform', 'improve', 'help', 'benefit', 'relief', 'forgiveness', 'cancel'
  ];
  
  const antiKeywords = [
    'oppose', 'against', 'reject', 'deny', 'no', 'stop', 'prevent', 'block',
    'unfair', 'wrong', 'bad', 'terrible', 'disaster', 'waste', 'burden'
  ];
  
  const emotionalIntensifiers = [
    'very', 'extremely', 'completely', 'totally', 'absolutely', 'definitely',
    'strongly', 'deeply', 'really', 'truly', 'highly', '!', 'crisis', 'urgent'
  ];
...
function generateMockEmbedding(text: string, sentiment: SentimentResult): number[] {
  ...
  const sentimentVector = sentiment.sentiment_class === 'pro' ? 1 : 
                         sentiment.sentiment_class === 'anti' ? -1 : 0;
