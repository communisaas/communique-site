...
export function getUserReputation(user: unknown): number {
	const userObj = user as UserWithReputation | null | undefined;
	return userObj?.reputation_score || userObj?.trust_score || 0;
}

export function getChallengerAddress(challenge: unknown): string | undefined {
	const challengeObj = challenge as ChallengeWithAddress | null | undefined;
	const challengeWithUser = challengeObj as unknown as { challenger?: { wallet_address?: string } };
	return challengeObj?.challenger_address || challengeWithUser?.challenger?.wallet_address;
}
