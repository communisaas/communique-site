<script lang="ts">
	...
	const providers = [
		{ provider: 'google', name: 'Google', color: '#4285F4', textColor: 'text-white' },
		{ provider: 'facebook', name: 'Facebook', color: '#1877F2', textColor: 'text-white' }
	];

	const secondaryProviders = [
		{ provider: 'twitter', name: 'X', icon: '𝕏', color: 'black' },
			...
			>
				{#if loadingProvider === auth.provider}
					<div
						...
						style="background-color: {auth.color}; opacity: 0.1;"
					></div>
					<Loader2 class="relative z-10 h-5 w-5 animate-spin text-slate-900" />
				{:else if auth.provider === 'linkedin'}
				...
				{:else}
					<div
						class="flex h-5 w-5 items-center justify-center rounded text-sm font-bold text-white"
						style="background-color: {auth.color}"
					>
						{auth.icon}
					</div>
