...
>
	...
	>
		...
		<div class="p-6">
			...
			<div class="mb-6 rounded-lg border border-slate-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-4">
				<h3 class="mb-1 font-semibold text-slate-900 text-sm">
					{template.title}
				</h3>
				<p class="mb-3 text-xs text-slate-600 line-clamp-2">
				...
				<div class="flex items-center gap-3 text-xs text-slate-500">
					...
					{#if template.metrics.sent > 100}
						<span class="text-blue-600 font-medium">📈 Growing momentum</span>
					{:else if template.metrics.sent > 50}
						<span class="text-green-600 font-medium">🎯 Building pressure</span>
					{:else}
						<span class="text-amber-600 font-medium">🚀 Early adopter</span>
					{/if}
				</div>
			</div>
			...
			<div class="mb-4 space-y-3">
				...
				{#each [{ provider: 'google', name: 'Google', icon: 'google-svg' }, { provider: 'facebook', name: 'Facebook', icon: 'f' }] as auth}
					...
					>
						...
						{:else}
							<div class="h-5 w-5 bg-[#1877F2] flex items-center justify-center rounded text-sm font-bold text-white">
								f
							</div>
						{/if}
				...
				<div class="grid grid-cols-3 gap-2">
					{#each [{ provider: 'twitter', name: 'X', icon: '𝕏', color: 'bg-black' }, { provider: 'linkedin', name: 'LinkedIn', color: 'bg-[#0077B5]' }, { provider: 'discord', name: 'Discord', color: 'bg-[#5865F2]' }] as auth}
						...
						>
							...
							{:else}
								<div class="h-5 w-5 {auth.color} flex items-center justify-center rounded text-sm font-bold text-white">
									{auth.icon}
								</div>
							{/if}
