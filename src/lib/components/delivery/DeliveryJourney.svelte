<!--
  ...
  - Uses groupByBody() for chamber/body grouping (instead of hardcoded senate/house)
  - Passes config down to child components (DeliveryConfirmation)
  - All orchestration logic and Svelte 5 patterns preserved
-->
<script lang="ts">
	...
	import { TIMING, LEGISLATURE_CONFIGS } from './delivery-types';
	import { computeSummary, groupByBody } from './delivery-utils';
	...
	interface Props {
		...
		/** Legislature configuration - defaults to US Congress */
	...
	/**
	 * Compute summary from results using shared utility
	 * ABSTRACTION: Uses computeSummary() instead of inline filtering
	 */
	let summary: DeliverySummary = $derived(computeSummary(results));

	/**
	 * Group results by legislative body (senate/house, or custom bodies)
	 * ABSTRACTION: Uses groupByBody() instead of hardcoded chamber filters
	 *
	 * This enables support for:
	 * - US Congress (Senate + House)
	 * - Unicameral legislatures (single body)
	 * - International parliaments (custom body names)
	 */
	let groupedResults = $derived(groupByBody(results, config.bodies.map(b => b.id)));
...
<div class="journey-container" data-stage={stage}>
	...
	{#if stage === 'complete' || stage === 'details'}
		<div class="stage-complete">
			<!-- ABSTRACTION: Pass config down to DeliveryConfirmation for headline customization -->
			<DeliveryConfirmation {summary} {config} />
