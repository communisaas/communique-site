<!--
  ...
  Shows grouped offices by chamber with status icons and outcome details.

  Based on: docs/temp/CWC-FEEDBACK-REDESIGN.md (Section 5D)

  Design Principles:
  - Secondary information, on-demand
  - Group by chamber (Senate first, then House)
...
<script lang="ts">
	...
	import { groupByChamber, formatLocation, getOutcomeLabel, formatConfirmationId } from './delivery-utils';
	...
	/**
	 * Results grouped by chamber (Senate first, then House)
	 */
	let grouped = $derived(groupByChamber(results));

	/**
	 * Whether we have senate results
	 */
	let hasSenate = $derived(grouped.senate.length > 0);

	/**
	 * Whether we have house results
	 */
	let hasHouse = $derived(grouped.house.length > 0);
	...
	>
		...
		{#if hasSenate}
			<div class="chamber-group">
				<h3 class="chamber-heading">Senate</h3>
				<ul class="office-list" role="list">
					{#each grouped.senate as result (result.office)}
		...
		{#if hasHouse}
			<div class="chamber-group">
				<h3 class="chamber-heading">House</h3>
				<ul class="office-list" role="list">
					{#each grouped.house as result (result.office)}
						<li class="office-item {getOutcomeColorClass(result.outcome)}">
							<div class="office-status">
								<span class="status-icon" aria-hidden="true">
									<svelte:component
...
<style>
	...
	.office-item.outcome-delivered {
		border-left-color: oklch(0.70 0.17 145); /* Green */
	}
