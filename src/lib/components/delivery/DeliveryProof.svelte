<!--
  ...
  Shareable proof/receipt card for legislature message delivery.
  ...
  - Compact office list grouped by legislative body
  - Multiple sharing options (copy, native share, download)
  - Legislature-agnostic via configuration
-->
<script lang="ts">
	...
	let receiptText = $derived(
		generateReceiptText(submissionId, results, config, templateTitle, timestamp)
	);
	...
	let displayRefId = $derived(
		submissionId.length > 16
			? `${submissionId.slice(0, 8)}...${submissionId.slice(-6)}`
			: submissionId
	);
		...
		if (canShare) {
			try {
				await navigator.share({
					title: `${config.name} Message Receipt`,
					text: receiptText
				});
			} catch (error) {
	...
	function handleDownload() {
		...
		link.href = url;
		link.download = `${config.name.toLowerCase().replace(/\s+/g, '-')}-receipt-${submissionId.slice(0, 8)}.txt`;
...
<article class="proof-card" aria-label="{config.name} Message Receipt">
	<!-- Header -->
	<header class="proof-header">
		<div class="header-icon">
			<FileText size={20} strokeWidth={1.5} />
		</div>
		<h2 class="header-title">Your {config.name} Message Receipt</h2>
	</header>
	...
	<!-- Office List (grouped by body) -->
	<section class="proof-offices">
		{#each config.bodies as bodyConfig}
			{@const bodyResults = groupedResults[bodyConfig.id] || []}
			{#if bodyResults.length > 0}
				<div class="body-group">
					<h3 class="body-title">{bodyConfig.displayName}</h3>
					<ul class="office-list">
						{#each bodyResults as result}
	...
	<footer class="proof-actions">
		...
		<button type="button" class="action-btn" onclick={handleShare} aria-label="Share receipt">
...
<style>
	...
	.proof-header {
		...
		background: linear-gradient(135deg, oklch(0.97 0.015 250) 0%, oklch(0.98 0.01 200) 100%);
