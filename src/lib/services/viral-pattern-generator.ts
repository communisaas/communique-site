...
function extractNumericalContrasts(text: string): ViralPattern['numerical_contrasts'] {
	...
	const billionPattern =
		/\$?([\d,]+(?:\.\d+)?)\s*(?:billion|million).*?vs.*?\$?([\d,]+(?:\.\d+)?)/gi;
	...
	const costPattern =
		/(\w+(?:\s+\w+)*)\s+(?:costs?|gets?|pays?|receives?)\s+\$?([\d,]+(?:\.\d+)?)/gi;
	...
	if (costMatches.length >= 2) {
		for (let i = 0; i < costMatches.length - 1; i += 2) {
			contrasts.push({
				...
				ratio:
					parseFloat(costMatches[i][2].replace(/,/g, '')) /
					parseFloat(costMatches[i + 1][2].replace(/,/g, ''))
			});
		}
	}
...
export function generateViralPattern(
	...
	const controversy_score =
		triggers.includes('moral_outrage') || contrasts.some((c) => c.ratio > 1000) ? 0.8 : 0.4;
