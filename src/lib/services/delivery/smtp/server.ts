...
export class DeliveryPlatformSMTP {
  ...
  private handleAuth(
    ...
    if (this.config.smtp.auth?.user && this.config.smtp.auth?.pass) {
      if (auth.username === this.config.smtp.auth.user && 
   ...
   */
  private async processIncomingMail(
    stream: Readable,
    session: SMTPServerSession
  ): Promise<void> {
    try {
      ...
      if (userResult.emailType === 'secondary' && !userResult.isVerified) {
        ...
        await this.bounceHandler.sendVerificationRequiredBounce(
          senderEmail, 
          templateIdentifier
        );
        return;
  ...
  public start(): void {
    ...
    this.server.listen(port, host, () => {
      ...
      console.log(`N8N Workflows: ${this.config.features.enableN8NWorkflows ? 'Enabled' : 'Disabled'}`);
      console.log(`VOTER Certification: ${this.config.features.enableVoterCertification ? 'Enabled' : 'Disabled'}`);
    });
