...
export class N8NClient {
	 ...
	 */
	public async triggerTemplateModeration(payload: N8NWebhookPayload): Promise<N8NWorkflowResult> {
		try {
	...
	public async triggerVerification(
		...
		try {
			const response = await this.client.post<VerificationResult>('/webhook/verification', {
				templateId,
				templateData,
	...
	public async triggerConsensus(
		...
		try {
			const response = await this.client.post<ConsensusResult>('/webhook/consensus', {
				verificationId,
				templateData,
				severity,
				timestamp: new Date().toISOString()
	...
	public async triggerReputationUpdate(
		...
		try {
			const response = await this.client.post<ReputationUpdate>('/webhook/reputation', {
				userId,
				userAddress,
				action,
				metadata,
	...
	public async monitorWorkflowPerformance(workflowName: string): Promise<{
		...
		try {
			...
			const successRate =
				metrics.total_executions > 0 ? metrics.successful_executions / metrics.total_executions : 1;
	 ...
	 */
	public async listWorkflows(): Promise<
		Array<{
			id: string;
			name: string;
			active: boolean;
