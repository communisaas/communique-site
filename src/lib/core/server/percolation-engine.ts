...
export async function buildInformationNetwork(): Promise<{
	...
	const nodes: NetworkNode[] = templateCreators.map((user) => {
		const totalCampaigns = user.templates.reduce(
			(sum, template) => sum + template.template_campaign.length,
			0
		);

		const successfulCampaigns = user.templates.reduce(
			(sum, template) =>
				sum + template.template_campaign.filter((c) => c.status === 'delivered').length,
			0
		);

		// Activation probability = success rate
		const activation_probability = totalCampaigns > 0 ? successfulCampaigns / totalCampaigns : 0;
	...
	userActivations.forEach((activatedUsers, sourceUserId) => {
		...
		activatedUsers.forEach((targetUserId) => {
			...
			if (sameDistrict)
				weight *= 2.0; // Same district = stronger connection
 ...
 */
export function calculatePercolationThreshold(nodes: NetworkNode[], edges: NetworkEdge[]): number {
 ...
 */
export function findNetworkBottlenecks(nodes: NetworkNode[], edges: NetworkEdge[]): NetworkEdge[] {
	// Find nodes with highest and lowest activation probability
	const sortedNodes = [...nodes].sort(
		(a, b) => b.activation_probability - a.activation_probability
	);
