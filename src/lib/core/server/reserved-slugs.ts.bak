...
const CORE_APP_ROUTES = ['api', 'auth', 'dashboard', 'demo', 'onboarding', 'template-modal'];
...
export function generateActionSlug(title: string, deliveryMethod: string): string {
	...
	if (deliveryMethod === 'certified') {
		...
		if (lowerTitle.includes('stop') || lowerTitle.includes('end') || lowerTitle.includes('ban')) {
			...
			lowerTitle.includes('demand') ||
			lowerTitle.includes('require') ||
 ...
 */
export async function suggestAvailableAlternatives(
	originalSlug: string,
	title: string,
	deliveryMethod: string,
	...
	if (availableSuggestions.length < 3) {
		...
		for (const prefix of prefixes) {
			...
			if (
				prefixedSlug !== originalSlug &&
				!isSlugReserved(prefixedSlug) &&
				(await checkDb(prefixedSlug))
			) {
...
export function generateUrgentVariations(baseSlug: string, deliveryMethod: string): string[] {
	...
	if (deliveryMethod === 'certified') {
		...
		variations.push(`demand-action-${baseSlug}`, `urgent-${baseSlug}`, `act-now-${baseSlug}`);
	}
