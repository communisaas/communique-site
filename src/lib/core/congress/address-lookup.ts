...
export class AddressLookupService {
    ...
    constructor() {
        ...
        if (!this.congressApiKey && env.NODE_ENV === 'production') {
            console.warn('CONGRESS_API_KEY environment variable is missing - Congress features will be disabled');
        }
    }
    ...
    private async getHouseRep(state: string, district: string): Promise<Representative> {
        try {
            ...
            console.log(`Looking for representative: ${stateAbbr}/${stateFullName} district ${districtNumber}`);
            ...
            const houseRep = allMembers.find((member: any) => {
                ...
                const isMatch = (member.state === stateAbbr || member.state === stateFullName) && memberDistrict === districtNumber;
            ...
            const senators = allMembers
                .filter((member: any) => {
                    ...
                    const isSenator = latestTerm?.chamber === 'Senate' || 
                                     (!member.district && latestTerm); // Senators don't have districts
