	...
	async submitMessage(data: {
		...
		for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
			try {
				...
				console.log(
					`Load Balancer: Selected TEE instance ${instance.id} (Attempt ${attempt}/${this.maxRetries})`
				);
				...
				console.warn(
					`Load Balancer: Submission failed (Attempt ${attempt}/${this.maxRetries})`,
					error
				);
		...
		throw new Error(
			`Failed to submit message after ${this.maxRetries} attempts. Last error: ${lastError?.message}`
		);
	}
}
