...
// Type definitions for our minimal Buffer implementation
...
class BufferShim extends Uint8Array {
	...
	static allocUnsafeSlow(size: number): BufferShim {
		// Same as allocUnsafe - "slow" version avoids internal pooling
		// which doesn't apply to our browser shim anyway
		return new BufferShim(size);
	}
	...
	// Slice returns a new BufferShim (not a view like Uint8Array.slice)
