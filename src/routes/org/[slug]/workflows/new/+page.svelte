...
<div class="min-h-screen bg-zinc-950 text-zinc-100">
	<div class="mx-auto max-w-4xl px-4 py-8">
		...
		<h1 class="mb-8 text-2xl font-bold text-zinc-100">New Workflow</h1>
		...
		<!-- Name + Description -->
		<div class="mb-6 space-y-4 rounded-lg border border-zinc-800/60 p-4">
			<div>
				<label for="wf-name" class="mb-1 block text-sm font-medium text-zinc-400">Name</label>
				<input
					id="wf-name"
					type="text"
					bind:value={name}
			...
			<div>
				<label for="wf-desc" class="mb-1 block text-sm font-medium text-zinc-400">Description (optional)</label>
				<input
					id="wf-desc"
					type="text"
		...
		<div class="mb-6 rounded-lg border border-zinc-800/60 p-4">
			<h2 class="mb-3 text-sm font-medium text-zinc-400">Trigger</h2>
			<select
			...
			{#if triggerType === 'tag_added'}
				<div class="mt-3">
					<label for="trigger-tag" class="mb-1 block text-xs text-zinc-500">Tag</label>
			...
			{#if triggerType === 'campaign_action'}
				<div class="mt-3">
					<label for="trigger-campaign" class="mb-1 block text-xs text-zinc-500">Campaign ID</label>
					<input
						id="trigger-campaign"
						type="text"
		...
		<div class="mb-6 rounded-lg border border-zinc-800/60 p-4">
			<div class="mb-3 flex items-center justify-between">
				<h2 class="text-sm font-medium text-zinc-400">Steps ({steps.length})</h2>
				<button
					onclick={addStep}
			...
			{#if steps.length === 0}
				<p class="py-6 text-center text-sm text-zinc-500">No steps yet. Add a step to define what happens when this workflow triggers.</p>
			{:else}
				<div class="space-y-3">
					{#each steps as step, i (i)}
						<div class="rounded-lg border border-zinc-700/50 bg-zinc-900/50 p-3">
							<!-- Step header -->
							<div class="mb-2 flex items-center justify-between gap-2">
								<span class="shrink-0 text-xs font-medium text-zinc-500">Step {i + 1}</span>
								<div class="flex items-center gap-1">
									<button
										onclick={() => moveStep(i, -1)}
										disabled={i === 0}
										class="rounded px-1.5 py-0.5 text-xs text-zinc-500 hover:text-zinc-300 disabled:opacity-30"
									>
										Up
									</button>
									<button
							...
							<select
								...
								class="mb-2 w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-500 focus:outline-none"
							>
							...
							{:else if step.type === 'delay'}
								<div class="flex gap-2">
									<input
										...
										min="1"
										class="w-24 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 focus:border-zinc-500 focus:outline-none"
									/>
									<select
								...
								<div class="space-y-2">
									<div class="flex gap-2">
										...
										<input
											...
											placeholder="Value"
											class="w-32 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 focus:border-zinc-500 focus:outline-none"
										/>
									</div>
									<div class="flex gap-2">
										<div class="flex-1">
											<label class="mb-1 block text-xs text-zinc-500">Then go to step</label>
											<input
												type="number"
										...
										<div class="flex-1">
											<label class="mb-1 block text-xs text-zinc-500">Else go to step</label>
											<input
												type="number"
