...
function analyzeViralCoefficient(template) {
  ...
  if (title.includes("vs") || title.includes("More Than") || title.includes("Less Than")) score += 10; // Contrast framing
...
function analyzeCongressionalAcceptance(template) {
  ...
  if (body.match(/\d+(?:,\d{3})*/) && !body.includes("I think") && !body.includes("I believe")) score += 10; // Fact-based
...
templates.forEach(template => {
  ...
  console.log(`   Viral Coefficient: ${viral}/100 ${viral >= 70 ? '🔥' : viral >= 50 ? '⚡' : '📈'}`);
  console.log(`   Congressional Acceptance: ${congressional}/100 ${congressional >= 80 ? '✅' : congressional >= 60 ? '⚠️' : '❌'}`);
  console.log(`   Balance Score: ${balance}/100 ${balance >= 60 ? '🎯' : balance >= 40 ? '⚖️' : '🔧'}`);
  
  // Recommendations
  if (viral < 50) {
    console.log(`   🔧 VIRAL FIX: Add numerical contrasts, "math doesn't work" opener, or forcing choice`);
  }
  if (congressional < 60) {
...
console.log(`   🔥 High Viral (70+): ${templates.filter(t => analyzeViralCoefficient(t) >= 70).length} templates`);
console.log(`   ⚡ Medium Viral (50-69): ${templates.filter(t => analyzeViralCoefficient(t) >= 50 && analyzeViralCoefficient(t) < 70).length} templates`);
console.log(`   📈 Low Viral (<50): ${templates.filter(t => analyzeViralCoefficient(t) < 50).length} templates`);

console.log('\\n🏛️ CONGRESSIONAL ACCEPTANCE BREAKDOWN:');
console.log(`   ✅ High Acceptance (80+): ${templates.filter(t => analyzeCongressionalAcceptance(t) >= 80).length} templates`);
console.log(`   ⚠️ Medium Acceptance (60-79): ${templates.filter(t => analyzeCongressionalAcceptance(t) >= 60 && analyzeCongressionalAcceptance(t) < 80).length} templates`);
console.log(`   ❌ Low Acceptance (<60): ${templates.filter(t => analyzeCongressionalAcceptance(t) < 60).length} templates`);
