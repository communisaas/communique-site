...
async function analyzeTestCoverage(): Promise<CoverageReport> {
  ...
  const criticalGaps = sourceAnalysis.filter(f => 
    !f.hasTest && 
...
function analyzeCriticality(filePath: string, content: string): SourceFile['businessCriticality'] {
  ...
  if (filePath.includes('/api/') || filePath.includes('/stores/templates') || filePath.includes('/congress/')) {
    return 'high';
  }
...
function generateRecommendations(sourceAnalysis: SourceFile[], criticalGaps: SourceFile[]): string[] {
  ...
  if (criticalGaps.length > 0) {
    ...
    criticalGaps.slice(0, 5).forEach(file => {
      recommendations.push(`   • ${file.path} (${file.businessCriticality} criticality, ${file.complexity} complexity)`);
    });
  }
...
async function main() {
  try {
    ...
    console.log(`\n📈 OVERALL COVERAGE:`);
    console.log(`   Files Tested: ${report.testedFiles}/${report.totalFiles} (${report.coveragePercentage}%)`);
    ...
    report.criticalGaps.slice(0, 10).forEach(file => {
      ...
      console.log(`     └─ ${file.businessCriticality} business impact, ${file.complexity} complexity`);
    });
