...
async function seedLegislativeChannels() {
  ...
  try {
    ...
    const channelsByType = createdChannels.reduce((acc, channel) => {
    ...
    const channelsByCountry = createdChannels.reduce((acc, channel) => {
    ...
    const bodiesByJurisdiction = createdBodies.reduce((acc, body) => {
