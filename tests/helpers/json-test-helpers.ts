  ...
  return new Proxy(obj, {
    get: (target, prop) => {
      ...
      if (value && typeof value === 'object' && value !== null) {
        ...
        const jsonFields = ['config', 'metrics_cache', 'properties', 'session_metrics', 'device_data', 'computed_metrics', 'multipliers'];
 ...
 */
export function safeExperimentConfig(experiment: any) {
...
export function safeExperimentMetricsCache(experiment: any) {
  const cache = experiment.metrics_cache || {};
  return {
...
export function safeEventProperties(event: any) {
  return event.properties || {};
}

export function safeSessionMetrics(session: any) {
...
export function safeDeviceData(session: any) {
  const data = session.device_data || {};
  return {
...
export function safeComputedMetrics(event: any) {
  const metrics = event.computed_metrics || {};
  return {
