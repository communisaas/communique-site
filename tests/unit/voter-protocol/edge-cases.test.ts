...
describe('Edge Cases: Boundary Conditions', () => {
	...
	describe('Authority Level Boundaries (1-5)', () => {
		...
		// NOTE: Document-type-to-authority mapping tests removed (didit-client deleted).
		// mDL via Digital Credentials API maps to authority level 5.

		it('should map mDL to authority level 5', () => {
			// mDL via Digital Credentials API is the sole verification path
	...
	describe('Timestamp Edge Cases', () => {
		// NOTE: Webhook timestamp tests removed (didit-client deleted).
		...
		it('should handle leap year birth date', () => {
			const birthYear = 2000; // Leap year
			const currentYear = new Date().getFullYear();
			const age = currentYear - birthYear;
			expect(age).toBeGreaterThanOrEqual(18);
		});
